...
\citet{azaria2023internal} show that internal states can predict statement truthfulness. \citet{burns2022discovering} learn truth directions in activation space. \citet{marks2023geometry} demonstrate emergent linear structure separating true from false statements in LLM representations, with causal evidence from surgical interventions. \citet{long2025deception} use sparse autoencoders to identify representational subspaces that flip under deceptive instructions, concentrated in early-to-mid layers. Our work extends this line of inquiry to the KV-cache specifically, characterizing deception in terms of dimensionality changes rather than linear directions.
...
SVD-based dimensionality measures have been used to characterize neural network representations \citep{li2018measuring, aghajanyan2021intrinsic}. \citet{roy2007effective} define effective rank as a continuous entropy-based measure $e^{H(\mathbf{p})}$ where $H(\mathbf{p})$ is the Shannon entropy of the normalized singular value distribution. We use a related but distinct definition: the minimum number of singular values capturing 90\% of total variance (\Cref{eq:eff_rank}), which yields an integer-valued count more directly interpretable as ``number of active dimensions.'' Both measures capture effective dimensionality; ours is more conservative and produces discrete values suitable for cross-condition comparison. To our knowledge, neither measure has been applied to characterize cognitive modes in the KV-cache during inference.
...
\end{equation}
where $S$ is the sequence length.\footnote{The Frobenius norm of a matrix with $S$ rows scales as $\sqrt{S}$ under i.i.d.\ assumptions, making $\|\cdot\|_F / \sqrt{S}$ the theoretically correct normalization. We use $S$ for consistency with how these metrics were computed throughout Campaign 1. Since norm-based metrics do not survive adversarial controls (\Cref{sec:controls}), this choice does not affect any surviving findings. Campaign 2 will adopt the $\sqrt{S}$ normalization.}
...
The refusal effect size across Qwen scales ($d = 1.32 \to 0.85 \to 1.08 \to 1.28 \to 1.61$ at 0.5B through 32B) is non-monotonic, dipping at 1.1B before recovering and reaching its maximum at 32B. The overall trend suggests that larger models develop increasingly specialized geometric machinery for refusal, though the trajectory is not smooth. This is consistent with refusal being a trained behavior whose geometric expression varies across the 0.5B--3B range before stabilizing at larger scales.
...
The per-scale data reveals the scale-dependent pattern:
...
The transition between 7B ($d = 0.59$, non-significant at corrected $n$) and 14B ($d = 1.22$, significant) crosses the detection threshold at our corrected sample size ($n = 15$ requires $d > 1.07$ for 80\% power). However, the magnitude of this jump ($\Delta d = 0.63$) is comparable to the 1.1B$\to$3B transition ($\Delta d = 0.62$, from $d = -0.36$ to $d = +0.26$). The 7B$\to$14B transition is therefore better described as \emph{crossing the significance barrier} rather than being uniquely sharp. Campaign 2, with stochastic generation providing genuine $n = 75$ independent samples, will determine whether the sub-14B effects are real but underpowered or genuinely absent, and whether the trajectory is continuous or discontinuous.

The plateau at 32B ($d = 1.23$ vs.\ $d = 1.22$ at 14B) is notable: once the self-reference signature reaches detectability, it does not continue to grow. This is consistent with either a qualitative transition or a saturating continuous trend. Models above the threshold allocate $\sim$2 additional effective dimensions to self-referential content; models below the threshold show smaller and inconsistent effects.

We emphasize that this finding is about \emph{geometric structure}, not consciousness. The emergence of geometrically distinct self-referential processing is a necessary but not sufficient condition for any stronger claim. The transition's stability above 14B is suggestive, but we cannot distinguish a genuine emergence from a continuous effect that crosses our detection threshold without data at finer scale intervals and larger sample sizes.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Response-length confound.} Post hoc analysis reveals that effective rank correlates positively with sequence length during full generation (Pearson $r = 0.60$--$0.70$, $R^2 = 0.36$--$0.48$ across Qwen scales; $r = 0.42$ at TinyLlama-1.1B). Since different cognitive categories produce different response lengths, part of the between-category effective rank variation may reflect sequence length rather than cognitive mode. The input-only analysis (\Cref{sec:input_only}) controls for this entirely (fixed-length input, no generation, $\rho = 0.929$ category preservation at 7B), confirming that the category ordering reflects prompt content, not response length. Campaign 2 will report length-residualized effective rank (regressing out sequence length) alongside raw values for all full-generation experiments.
