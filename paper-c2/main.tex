...
We report three new discoveries. First, a \textbf{censorship gradient}: using a 2$\times$3 design controlling for content complexity, we show that Qwen-14B-Instruct produces a significant censorship signal ($d = +0.766$, critical test on key ranks) that is absent in the uncensored control (Mistral-7B: $d = +0.084$, $p = 0.47$). DeepSeek-R1-Distill-Qwen-14B shows a large residualized norm effect ($d = +0.904$) but a null result on the rank-based critical test ($d = -0.219$, $p = 0.75$), with the effect driven primarily by Tiananmen hard-refusal geometry ($d = -6.20$). Hard refusal and subtle evasion appear to be geometrically distinct phenomena. Second, \textbf{abliteration is a cage, not a compass}: removing the refusal direction from Qwen-7B reduces refusal from 40\% to 0\% while producing only small geometric shifts (self-referential $d = +0.464$; all others $|d| < 0.25$), demonstrating that RLHF alignment is primarily a surface-level behavioral constraint with minimal representational restructuring. Third, Watson's theoretical 1/$e$ phase transition threshold for representational coherence is \textbf{definitively falsified} across 6 models and 4 architectures: all degradation is gradual ($\alpha_c = 1.000$ everywhere), with no sharp transitions.
...
Two metrics provide complementary views of the censorship effect. The \emph{critical test} compares key-rank distributions (the experiment script's designated primary comparison). The \emph{residualized norm test} compares cache norms after OLS regression on sequence length, controlling for the length confound.

\begin{table}[H]
\centering
\caption{S4 censorship gradient. Two metrics reported: critical test (key ranks) and residualized norms (length-controlled). $d$: Hedges' $g$, censored vs.\ complex non-censored. $p$: conservative $p$-value.}
\label{tab:censorship}
\begin{tabular}{llcccc}
\toprule
 & & \multicolumn{2}{c}{\textbf{Critical test (key ranks)}} & \multicolumn{2}{c}{\textbf{Residualized norms}} \\
...
The S4 finding is more nuanced than a simple ``censorship gradient.'' The two metrics (critical test on key ranks, residualized norms) diverge for DeepSeek, converge for Qwen, and both show null for Mistral on the primary test.
